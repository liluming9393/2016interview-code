...
int main()
{
	vector<string> data1 = { "1", "2", "3", "4", "5", "6", "7"};
	TreeNode* root1 = CreateBinaryTree(data1);
	vector<string> data2 = { "3", "6", "7" };
	TreeNode *root2 = CreateBinaryTree(data2);
	bool result = chkIdentical(root1, root2);
	printf("%d\n", result);
	system("pause");
	return 0;
}
...
bool chkIdentical(TreeNode* A, TreeNode* B) 
{
	...
	return isSameTree(A, B) || chkIdentical(A->left, B) || chkIdentical(A->right, B);
}
bool isSameTree(TreeNode *A, TreeNode *B)
{
